...
import static io.myfinbox.spendingplan.DataSamples.newSampleCretePlanResource
...
class PlanControllerSpec extends Specification {
    ...
    def cleanup() {
        JdbcTestUtils.deleteFromTables(jdbcTemplate, 'spendingplans')
    }

    def "should create a new spending plan"() {
        given: 'user wants to create a spending plan'
        var request = newSampleCretePlanResource()
    ...
    def postPlan(String req) {
        restTemplate.postForEntity('/v1/spending-plans', entityRequest(req), String.class)
    }
